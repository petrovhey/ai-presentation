// popup.js - Mini dashboard for current tab + Quick Ask
let currentTabUrl = '';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('openSidepanel').addEventListener('click', async () => {
    try {
      ...
      console.error('Failed to open sidepanel:', e);
    }
    window.close();
  });

  document.getElementById('quickAskBtn').addEventListener('click', async () => {
    ...
    try {
      await chrome.storage.session.set({ quickAsk: prompt });
      const win = await chrome.windows.getCurrent();
      await chrome.sidePanel.open({ windowId: win.id });
      window.close();
    } catch (e) {
  ...
  document.getElementById('quickInput').addEventListener('keydown', (e) => {
    if (e.key === 'Enter') {
      document.getElementById('quickAskBtn').click();
    }
  });
});
